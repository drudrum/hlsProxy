...
module.exports=function(options,parent,listName){
  ...
  options.cookieUrl && request.get({
    url:options.cookieUrl,
    encoding:null,
    timeout:20000,
    jar:cookieJar
  },(err,resp,rdata)=>{
    //console.log('get cookies',cookieJar.getCookieString(options.cookieUrl));
  });
...
//      console.log("Send cached file ",url);
  ...
  function getList(){
    ...
    var nowSec=Math.floor(rtime/1000);
    var playlistUrl=options.playlist.replace(/\$nowSec/g,String(nowSec))+'?'+rtime;
